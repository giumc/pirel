...
from tools import LayoutDefault,LayoutTool,Point,Line
...
class LayoutPart(ABC) :
    ...
    def get_area(self):
        ...
        cell_dummy=self.draw()
        cell_dummy.name=cell_dummy.name+"Dummy"
        area=cell.get_bounding_box()

        return Point(area[1,0]-area[0,0],area[1,1]-area[0,1])
...
class IDT(LayoutPart) :
    ...
    def draw(self):
        ...
        r1=gdspy.Rectangle(o.get_coord(),\
            (o+self.get_finger_size()).get_coord(),\
            layer=self.layer)

        r2=gdspy.copy(r1,self.pitch,self.y_offset);

        unitcell=gdspy.Cell(self.name+"single")
        ...
        if self.n==1:
            ...
            cellarr=gdspy.CellArray(unitcell,self.n,1,[2*self.pitch,0])

            cell_out=gdspy.Cell(self.name+"multiple")

            cell_out.add(gdspy.PolygonSet(cellarr.get_polygons(),\
                layer=self.layer))
    ...
    def get_anchor_lines(self):
        ...
        o=self.origin
        o1=o+Point(self.pitch,+self.y+self.y_offset)
        dx_cov=Point(self.pitch*self.coverage,0)
        l1=Line(o,o+dx_cov)
        dx_dy_cov=dx_cov.get_coord()*(Point(self.n,1).get_coord())
        l2=Line(o1,Point(o1+Point(dx_dy_cov[0],dx_dy_cov[1])))
